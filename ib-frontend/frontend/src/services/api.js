...
// Por defecto en este proyecto usamos el backend real cuando está disponible
const OFFLINE_MODE = (import.meta.env.VITE_OFFLINE_MODE ?? 'false') === 'true';
...
const getOfflineResponse = (config) => {
  ...
  if (url === '/clientes/admin' && method === 'get') {
    return { status: 200, data: { clients: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } } };
  }
...
export const clientService = {
  ...
  getAdminCreatedClients: async () => {
    // Nuevo endpoint en backend: /clientes/admin
