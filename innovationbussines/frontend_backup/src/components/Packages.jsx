...
const Packages = () => {
  ...
  if (loading) {
    return (
      <section className="py-20 bg-gradient-to-b from-gray-900 via-amber-950 to-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-amber-200">Cargando paquetes...</p>
          </div>
        </div>
      </section>
    )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {packages.map((pkg, index) => {
            // Adaptar datos de diferentes formatos
            const precio = typeof pkg.price === 'string' ? pkg.price : `Desde $${pkg.precio || pkg.price}`;
            const duracion = pkg.duration || pkg.duracion;
            const grupo = pkg.group || pkg.grupo;
            const calificacion = pkg.rating || pkg.calificacion || 4.5;
            const imagen = pkg.image || pkg.imagen;
            const tipo = pkg.type || pkg.tipo || 'Internacional';
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 flex items-center gap-1 bg-amber-600/90 px-2 py-1 rounded-full">
                    ...
                    <span className="text-sm font-semibold text-amber-100">{calificacion}</span>
                  </div>
                </div>

                {/* Package Content */}
                <div className="p-6">
                  <h3 className="text-xl font-bold text-amber-200 mb-3">{pkg.name || pkg.nombre}</h3>
                  <p className="text-amber-100 mb-4 line-clamp-3">{pkg.description || pkg.descripcion}</p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-2xl font-bold text-amber-300">{precio}</div>
                    <button
                      onClick={() => openWhatsApp(pkg.name || pkg.nombre)}
